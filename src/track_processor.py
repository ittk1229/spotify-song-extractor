...
class TrackProcessor:
    ...
    def __init__(self, spotify_client: SpotifyClient, cache_manager: CacheManager, use_cache: bool = True):
    ...
    def get_all_artist_tracks(self, artist_id: str) -> list[tuple[str, str, str]]:
        ...
        if cache_result is not None:
            ...
            print(f"💾 キャッシュから楽曲情報を読み込みました ({len(cached_tracks)} 曲)")
            ...
            if new_tracks:
                ...
                if updated_tracks != cached_tracks:
                    ...
                    print(f"💾 楽曲情報を更新しました (新規追加: {len(updated_tracks) - len(cached_tracks)} 曲, 合計: {len(updated_tracks)} 曲)")
    ...
    def _merge_tracks(self, cached_tracks: list[tuple[str, str, str]], new_tracks: list[tuple[str, str, str]]) -> list[tuple[str, str, str]]:
        """キャッシュされたトラックと新しいトラックをマージ（重複削除）"""
        existing_ids = {track[0] for track in cached_tracks}
        unique_new_tracks = [track for track in new_tracks if track[0] not in existing_ids]
    ...
    def _get_new_tracks_since(self, artist_id: str, last_updated: str) -> list[tuple[str, str, str]]:
        ...
        print(f"🔄 {last_update_date.strftime('%Y-%m-%d %H:%M')} 以降の新しい楽曲を検索中...")
    ...
    def _filter_albums_by_date(self, albums: list[dict], cutoff_date: datetime) -> list[dict]:
        """指定日以降のアルバムをフィルタリング"""
        relevant_albums = []
    ...
    def _extract_tracks_from_albums(self, albums: list[dict]) -> list[tuple[str, str, str]]:
        """アルバムリストからトラック情報を抽出"""
        all_tracks = []
    ...
    def filter_tracks_by_keyword(self, tracks: list[tuple[str, str, str]], keyword: str) -> list[tuple[str, str, str]]:
        """キーワードでトラックをフィルタリング"""
        filtered_tracks = [
            track for track in tracks
            if keyword.lower() in track[1].lower()
        ]
        print(f"🔍 キーワード '{keyword}' に一致する楽曲を {len(filtered_tracks)} 曲見つけました")
        return filtered_tracks

    def get_new_tracks_for_playlist(self, tracks: list[tuple[str, str, str]], existing_track_ids: set[str]) -> list[tuple[str, str, str]]:
